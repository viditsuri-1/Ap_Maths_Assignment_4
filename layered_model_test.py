...
def test_define_dense_model_single_layer():
    ...
    assert len(model.layers) == 2, "Model should have 2 layers (Input and Dense)"
    assert model.layers[0].input_shape == (None, 43), "Input shape is not correct"
    assert model.layers[1].output_shape == (None, 1), "Output shape is not correct"
...
def test_define_dense_model_with_hidden_layer():
    ...
    assert len(model.layers) == 3, "Model should have 3 layers (Input, Dense hidden, Dense output)"
    assert model.layers[0].input_shape == (None, 43), "Input shape is not correct"
    assert model.layers[1].output_shape == (None, 11), "Hidden layer output shape is not correct"
    assert model.layers[2].output_shape == (None, 1), "Output shape is not correct"
